...
#    local('cd ~; touch test106.txt')
    ...
    # copy site
    local('sudo mkdir -p /etc/nginx/sites-available')
    local('sudo mkdir -p /etc/nginx/sites-enabled')
    local('sudo mkdir -p /etc/nginx/conf.d')
    ...
    local('sudo scp pabb@pabb-608:/etc/nginx/conf.d/* /etc/nginx/conf.d')
